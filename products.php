  ...
  <main class="max-w-6xl mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 stagger-children">
      ...
      $sql = "SELECT p.*, c.name as category_name, c.slug as category_slug, 
              (SELECT image_path FROM product_images WHERE product_id = p.id ORDER BY is_primary DESC, display_order ASC LIMIT 1) as image_path 
              FROM products p 
              LEFT JOIN categories c ON p.category_id = c.id 
              WHERE p.status = 'active'";

      if ($category_filter) {
        $sql .= " AND (c.slug = '$category_filter' OR c.name = '$category_filter')";
      }
      ...
      if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          ...
          $price = htmlspecialchars($row["price_start"] ?? 'Custom');
          $material = htmlspecialchars($row["material"] ?? 'Premium Wood');
          $sizes = htmlspecialchars($row["dimensions"] ?? 'Custom');
          $image = htmlspecialchars($row["image_path"] ?? 'frontend/images/placeholder.jpg');
          $link_name = urlencode($row["name"]);
      ?>
          <article data-category="<?php echo $category_slug; ?>"
